...
encoding_dimension = len(Alist) + 3
...
# help functions
def mz2pos(mz, pre=precision):
    return int(round((mz - MZ_START) / pre))


def pos2mz(pos, pre=precision):
    return pos * pre + MZ_START
...
INPUT_SHAPE = [-1, INPUT_DIMENSION]


# embed input item into a matrix
def embed(
    spectrum, shape=INPUT_SHAPE, mass_scale=200, embedding=None, ignore=False, pep=None
):
    if embedding is None:
        embedding = np.zeros(shape, dtype="float32")
...
def preprocessor(batch):
    ...
    embedding = np.zeros((batch_size, *INPUT_SHAPE), dtype="float32")
...
def tomgf(sp, y):
    ...
    imz = (
        np.arange(0, SPEC_DIMENSION, dtype="int32") * precision + MZ_START
    )  # more acurate
...
parser.add_argument("--batch_size", type=str, help="batch size per loop", default=256)
parser.add_argument(
    "--output", type=str, help="output file path", default="example_prediction.mgf"
)
    ...
    inputs.append(
        {
            ...
            "title": item.Peptide,
            "nce": item.NCE,
            "type": types[item.Type.lower()],
            "mass": fastmass(pep, "M", item.Charge, mod=mod),
        }
    )

    INPUT_SHAPE[0] = max(
        INPUT_SHAPE[0], len(pep) + 2
    )  # update xshape to match max input peptide
...
while len(inputs) > 0:
    ...
    y = pm.predict(
        input_generator(sliced_spectra, preprocessor, batch_size=args.batch_size),
        verbose=1,
    )
    y = np.square(y)
